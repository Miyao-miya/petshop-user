...
<template>
    <div class="login" id="login">
        ...
        <div class="log-email">
            <input type="text" placeholder="账号" :class="'log-input' + (account==''?' log-input-empty':'')"
                   v-model="account">
            <input type="password" placeholder="密码" :class="'log-input' + (password==''?' log-input-empty':'')"
                   v-model="password">
            <a  class="log-btn" @click="reservations"
               style="float: left; width: 30%; margin-left: 10%">注册</a>
            <a href="javascript:;" class="log-btn" @click="login" style="float: right;  width: 30%; margin-right: 10%">登录</a>
        </div>
...
<!--        注册组件-->
        <el-dialog
            ...
            <UserInsert/>
            <el-button @click="dialogVisiblePhoto = false"
            style="float: right; margin-right: 12%">取 消</el-button>
            <p>&nbsp;</p>
            <p>&nbsp;</p>
        </el-dialog>
    </div>
        ...
        methods: {
            ...
            login() {
                if (this.account != '' && this.password != '') {
                    ...
                    _this.$api.userApi.login(login,login.uId).then(res =>{
                        if (res == 1){
                            window.sessionStorage.setItem('pkId',login.uId)
                            this.$emit("logins",login.uId)
                            // this.$router.go(0)
                        }else alert("失败")
                    })
                }
